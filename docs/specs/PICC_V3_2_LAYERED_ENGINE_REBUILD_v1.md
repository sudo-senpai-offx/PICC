...
| T3 | 15s fast pipeline slice + HUD state snapshot ((b)/(c) cadence) — **DEFERRED** (no 15s plane probe; a 15s-slot plan gates on the ccxt ladder that starts at 60s, ADR-0003) | liveEO 15s plane probe |
| T4 | Futures-proxy leg + validator (>0.80 corr / >70% same-sign) — **DEFERRED** with T3 | T3 |
| T5 | Δ/CVD/rel-vol crypto wiring — **pillar half ✓ shipped** (`v32Execution.mjs`: `volumeDelta`/`cumulativeVolumeDelta` pure, `relativeVolume` bar-volume; 20 tests). **Feed half pending:** no ccxt trades feed exists (`liveCCXT.volumeProxy` is bar-activity, not maker flow) ⇒ honest `reason:"no trades feed"`, never synthesized | ccxt trades feed |
| ~~T6~~ ~~VWAP dual-anchor + EMA 9/21 pillars~~ ✓ | `v32Execution.mjs` `vwapPillar` (anchored-cumulative primary == `indicators.vwap` fixture; session-reset secondary from session-open bar; <15 bars honest-close) + `emaPair` (9/21 alignment/spread/<21 honest-close); 20 tests | — |
...
| ~~T10~~ ~~Cost line + trip-wire 5 (EV margin abort) + trip-wire 8 setting~~ ✓ | v3.2 `costLine` == `constitution.costAdjustedEv().line` semantics on every forex/EO output (`v32Engine.mjs`, REQ-P3-6); trip-wire 5 = EV-RR `< 2` abort on the cost line (`v32Copilot.mjs` wire 5, 36 tests); trip-wire 8 setting = persisted `proposalCap` (0 = unlimited, `v32Config.mjs`) | T2 |
...
| T12 | Re-point demoted-trigger tests; honesty pins — **honesty-pin half ✓ shipped** (REQ-P3-12 sweep, `v32Honesty.test.mjs` 17: every exported v3.2 shape has `available`/`source`, every unavailable path has a non-empty `reason`, no fabricated feed). **Demoted-trigger re-pointing stays pinned** for the soak (plan-2's global pin, ADR-0004 consent) — a v3.2 flip depends on it, not the other way | T2 |
| T13 | 15s studio-leg real-venue E2E (carried from U4FA T13) — **DEFERRED** with T3 | T3 |