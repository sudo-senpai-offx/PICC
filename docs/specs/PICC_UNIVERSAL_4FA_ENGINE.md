...
| Extension session-sync E2E unverified (user-flagged open item) | Confirmed open: `extensionSessionCapture.test.mjs` drives a **mock API harness** (no real venue tab); popup "session synced" is a literal label on `POST /api/trading/capture-session` ok (`popup.js:45` + `sourceLeg` badge `popup.js:105-109`; handler `handlers.mjs:4397`; provenance `captureProfiles.mjs:478,580`) | T13 (live real-tab E2E) is a **blocking edge** for any U4FA feature consuming real venue data (T14), exactly as the user flagged |
...
```json
{
  ...
  "honesty": { "spreadSource": null, "structureSource": "aggregate-h4", "calendarSource": "feed", "candleSource": "liveEO-extension" }
}
```
...
| T13 **Live E2E venue-session acceptance test (real tab → popup → server → decisions)** | P1 | L | T14 | — |
...
Human-run, Part-B style (see `docs/TRADING_RUNBOOK.md` Part-B pattern — **UNVERIFIED exact layout, check at execution**): on ONE real, logged-in venue tab (EO demo): (1) extension popup shows the venue row with `sourceLeg` and the ok status (`popup.js:45,105-109`); (2) popup row appears and the server ACKs the saved session (`POST /api/trading/capture-session`, `handlers.mjs:4397`); (3) live venue data flows — `getBrokerData` shows the real asset, `periods[300]` advances, `getSessionLive()` reports the extension leg (`autopilot.mjs:901-905`); (4) U4FA evaluation over that real data produces verdicts with honest `candleSource:"liveEO-extension"`. **Acceptance:** all four steps pass and are logged (`docs/T11_E2E_MANUAL_LOG.md` is the existing log format — reuse it); any failure here **blocks T14** and any real-data-consumption claim in the UI; step (4) is repeatable for ≥15 min of live bars. This is the user-flagged open item — the mock-API tests (`extensionSessionCapture.test.mjs`) prove the contract, not the venue reality. *Blocks T14.*
...
- **R7: real-tab E2E (T13) may find the extension's session-sync is cosmetic.** The user flagged this; it is exactly what T13 exists to reveal. If step (2)/(3) fail on a real logged-in tab, the fix is in the extension/worker relay, and **no U4FA real-data feature ships until it passes** — this is a gating edge, not a design risk to work around.
