...
- [ ] **A4 — F5 fix.** Validate `timeframe` key in `handlers.mjs:1724` path. **Acceptance:** bogus tf → 400 `{error:"unsupported timeframe"}` with unit test. *(OPEN — honest fallback shipped; strict 400 validation SCHEDULED in ledger F5 — needs whitelist audit for EO watch keys + chart tf labels + "daily".)*
- [ ] **A5 — F6 doc purge.** Rewrite the four doc sections to match shipped behavior (post-Phase 1). **Acceptance:** `docs-vs-code` grep check in PR (declared claims × code search) has zero mismatches; `TRADING_RUNBOOK.md:52-53` no longer instructs the reload that triggers invalidation. *(PARTIAL — README/ARCHITECTURE truth-synced 2026-09-03; SETUP/RUNBOOK/PRIVACY dual-mode scheduled, ledger F-12.)*
