...
14. **The e2e harness was loading the repository `.env` into the test server — a real credential exposure, found in review and fixed.** `server/config.mjs:11-21` calls `process.loadEnvFile()` unless `PICC_ENV_LOADED` is set, and Playwright *merges* the parent environment with `webServer.env` rather than replacing it — so redirecting the data directories did **not** make the run credential-free. Proven empirically (variable names only, never values): without the flag the e2e server loaded **2 credential-bearing vars, a complete `PICC_CCXT_WALLETADDRESS_HYPERLIQUID` + `PICC_CCXT_PRIVATEKEY_HYPERLIQUID` wallet pair**; with `PICC_ENV_LOADED=1` it loads **0**. The harness now sets that flag, asserts it is exactly `"1"`, refuses to start if the parent process exports any `PICC_CCXT_*`, and ships the AC-2b negative containment tests. R3.3's "never configures CCXT vars" was therefore false as first written and is now true by construction.
15. **The startup-health readout is gated more strictly than the rest of the app, by owner decision.** `requireAuth()` treats any loopback peer as trusted and also bypasses when no users exist. That is correct for the local single-user dev flow, but this readout names configured exchanges, token age, and rail mode, and behind the supported reverse-proxy/tunnel setup the proxy itself is loopback — so that bypass would hand operational credential metadata to an unauthenticated remote caller. The route now uses a route-local `requireAuthStrict` that accepts **only** a real session or bearer token. Unauthenticated loopback now returns 401; the other command-centre routes are deliberately unchanged.
16. **R3.1's execute-consent assertion is not satisfiable in a credential-less run, and the spec is amended to match reality.** The flow is: kill-switch toggle → gate deny (`blockedBy === "kill-switch"`), then rearm → propose → `fresh-data`. With no venue credentials the proposal never reaches an open/executable state, so the UI renders **zero** execute controls and the execute-consent POST is never reachable. The e2e asserts that honest terminal state (no execute button, no `execution:executed` audit row) instead of pretending to drive a consent modal. Driving the credential refusal end-to-end would require seeding a synthetic open proposal, which is a fixture decision beyond WS-5's scope. **Flagged for owner review.**
17. **D5's boot semantics now hold on the dev/e2e path too.** `index.mjs` ran the health check at boot, but vite imports `handlers.mjs` without `index.mjs`, so the *first GET* performed the "boot" work and wrote the `audit:startup-health` row lazily. `vite.config.ts` now runs it during startup (guarded, non-fatal) and the route serves the cached boot result.
...
- **T6 final observed 2026-09-24 (GREEN, post-review-fix):** (1) `npx vitest run --maxWorkers=1` = **281 files passed / 1 skipped (282); 3133 tests passed / 1 skipped (3134)** - above the 278/3102 WS-4 baseline; 242s. (2) `npm run typecheck` = **exit 0**. (3) `verifyAudit()` = **`{"ok":true,"brokenAt":null,"reason":null}`**. (4) `npm run test:e2e --workspace @picc/dashboard` = **4 passed**, including a fresh run after `rm -rf .playwright-tmp` (AC-2a) and **no writes under `apps/dashboard/server/data`** (AC-3c). (5) `npx vitest run server/__tests__/ws5SeamGuard.test.mjs --maxWorkers=1` = **12 passed**.
- **Review round (security + code) outcomes:** the e2e `.env` credential exposure (honesty note 14) and the advisory-boot-blocking audit append were **fixed and covered by tests**; the loopback auth bypass on this readout was **closed** (note 15); the TTL floor-to-days bug was fixed with boundary tests; AC-2b's missing negative containment test was added; the startup-health fixtures now use real encrypted `pva1` envelopes including a tampered-ciphertext case, so D5's "stores decrypt" is genuinely exercised; the seam guard now pins all three lock call sites. R3.1's execute-consent assertion was amended (note 16) rather than faked.
