...
**Files:** `apps/dashboard/src/terminal/adapters/realtime.ts`, `apps/dashboard/src/terminal/adapters/venueIntegrity.ts`, `apps/dashboard/src/terminal/components/StatusBoundary.tsx`, `apps/dashboard/src/terminal/components/CopilotPanel.tsx`, `apps/dashboard/src/terminal/hooks/useTerminalSnapshot.ts`, `apps/dashboard/src/terminal/contracts.ts`, `apps/dashboard/src/hooks/useRealtimeSuite.ts` (only adapter-level integration), `apps/dashboard/server/handlers.mjs` (read-only route additions only if required).
