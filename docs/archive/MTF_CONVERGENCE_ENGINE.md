...
- [x] 5c. RSI 60/40 as an **intraday-preset option** (giua64 Intraday) iff needed — never StochRSI. **Acceptance:** if added, labeled RSI 60/40 not StochRSI. — **Done (2026-09-19):** the intraday preset now reads momentum from the strict RSI 60/40 band. `RSI_TRIGGER_BAND={lo:40,hi:60}` exported; `voteMomentumRsi60_40` labels reads "RSI 60/40 bull/bear/neutral" — never "StochRSI"; a "RSI 60/40" reason-string test plus a banned-string guard pin the labeling. `PRESET_MOMENTUM.intraday={mode:"rsi60_40"}` (all other presets null → keep the default RSI>50 + MACD read); `resolvePreset` surfaces `.momentum`; `planeScore`/`converge` thread a `momentumRsi` flag; `marketConvergence.mjs` (the live intraday ladder) passes it through so the section honors the preset. RSI-only (MACD does not participate — deliberate: the band is the RSI trigger, per R7: "an RSI 60/40 read, never StochRSI"). Tests: voter unit (61→+1, 39→−1, 40/50→0), planeScore/converge threading, preset-option assertions. Full suite 223 files / 2341 tests green; typecheck clean.
