...
| **F5 — Indicators timeframe key unchecked** | `handlers.mjs` `/api/trading/indicators` | Bogus/unserved timeframe silently degrades | Partial fix (honest fallback) + **strict validation scheduled** | **A4 closed 2026-09-04 (`e2a8d7c`)**: strict 400 `{error:"unsupported timeframe"}` on unknown keys; whitelist audited to liveEO watch-period keys + short labels + daily family; per-candle honest labels unchanged | ✅ CLOSED (A4 commit + `__tests__/indicatorsTimeframe.test.mjs`) |
| **F6 — Doc drift** | `README.md`, `docs/SETUP.md`, `docs/TRADING_RUNBOOK.md`, `docs/ARCHITECTURE.md` | Claims describing dead architecture; PDPA exposure via `PRIVACY.md` | Truth-sync live docs now; `PRIVACY.md` dual-mode rewrite scheduled | README/ARCH corrected (2026-09-03); SETUP credential claim vault-truth-synced, RUNBOOK verified current, PRIVACY dual-mode rewritten (2026-09-04 docs batch) | `PRIVACY.md` (dual-mode), `docs/SETUP.md` §7b, `docs/TRADING_RUNBOOK.md` (full read, current) | ✅ CLOSED (2026-09-04 docs batch) |
...
| **F8 — Prior-plan leftovers** | `docs/TRADING_MULTIPLATFORM_ROADMAP.md` §7 "still queued" | Un-triaged debt | Triage into ledger | Portfolio card + correlation-screened portfolio → **landed 2026-09-04 strategy wave** (`CorrelationScreen` + pre-existing `PortfolioAggregatePanel`); latency-stats surface (`dataBusStats()` collector) → **landed** in the T-series broker rows; multi-platform autopilot routing → **declined by design** (advisory-only lane, per `TRADING_ROADMAP.md` §7 note) | ✅ CLOSED (triage recorded; scheduled items landed) |
...
| **F-11 — Volatility discards intraday OHLC** | volatility estimator (std of log returns only) | Garman-Klass/Yang-Zhang roughly halve estimator error | **Closed 2026-09-04 (`e1ab756` + strategy wave)**: Garman-Klass + Yang-Zhang estimators added and wired into `volatilitySnapshot`; same-sample tests prove they capture intraday range + overnight gaps that std-only (and high-low-only) miss. **Strategy wave (same day):** `annualizedVolatility` estimator chooser exported from `volatility.mjs` and wired into the autopilot position-sizing consumer; correlation screen shipped in the suite (`CorrelationScreen` over `/api/trading/correlation`, incl. the pre-existing portfolio card's pre-trade risk check) | `__tests__/volatilityModel.test.mjs` (F-11 block); `__tests__/volatility.test.mjs`; `src/components/CorrelationScreen.tsx` | ✅ CLOSED (estimators + live consumer wiring + correlation screening in suite) |
| **F-12 — Stale docs / PRIVACY drift** | `PRIVACY.md` (dead local-only arch), `README.md` claims, spec checkbox state | PDPA exposure; readers told a fiction | README/ARCH truth-synced (2026-09-03); **PRIVACY dual-mode rewrite** landed (2026-09-04, local-only vs hosted/Supabase, vault-at-rest truth); spec-checkbox reconciliation done across the Q5 + explicit-audit specs | `PRIVACY.md` (rewrite); spec sets carry ticked boxes | ✅ CLOSED (2026-09-04) |
