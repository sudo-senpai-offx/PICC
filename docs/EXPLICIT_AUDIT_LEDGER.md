...
| **F-11 — Volatility discards intraday OHLC** | volatility estimator (std of log returns only) | Garman-Klass/Yang-Zhang roughly halve estimator error | **Closed 2026-09-04 (`e1ab756`)**: Garman-Klass + Yang-Zhang estimators added and wired into `volatilitySnapshot`; same-sample tests prove they capture intraday range + overnight gaps that std-only (and high-low-only) miss | `__tests__/volatilityModel.test.mjs` (F-11 block) | ✅ CLOSED (estimators + tests; live autopilot/riskParity wiring still scheduled for the strategy-program wave) |
